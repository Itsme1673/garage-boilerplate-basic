...
export default function SignUpPage() {
  ...
  const onSubmit = async (data: SignupInput) => {
    try {
      ...
      if (error instanceof Error && error.message.includes('email-already-in-use')) {
        showErrorToast('Email already in use', 'An account with this email already exists.')
      } else {
        showErrorToast('Sign-up failed', 'Failed to create account. Please try again.')
      }
    }
  }
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-6xl font-bold tracking-tight text-zinc-900 whitespace-nowrap">Create account</h1>

      <div className="space-y-6 rounded-lg border border-zinc-200 bg-white p-8 shadow-sm md:mt-[60px] md:-mr-12 md:translate-x-[345px]">
        <button
          type="button"
